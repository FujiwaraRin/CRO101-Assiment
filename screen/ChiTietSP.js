...
const ChiTietSP = ({ route }) => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      try {
        const response = await fetch('http://192.168.1.12:3000/carts');
  ...
  const handlePress = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://192.168.1.12:3000/products/${item.id}`, {
  ...
  const handlePressCart = async () => {
    ...
    if (existingItemIndex !== -1) {
      ...
      try {
        const response = await fetch(`http://192.168.1.12:3000/carts/${existingItem.id}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
          },
