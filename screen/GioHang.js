...
const GioHang = (props) => {
  ...
  const getListPro = async () => {
    let url_api = 'http://192.168.1.12:3000/carts';
    ...
    const handleMinusPress = async (item, size) => {
      try {
        ...
        if (updatedDssp[selectedItemIndex].prices[selectedPriceIndex].quantity > 0) {
          ...
          await fetch(`http://192.168.1.12:3000/carts/${item.id}/sizes/${size}`, {
            method: 'PUT',
            headers: {
    ...
    try {
      await Promise.all(
        dssp.map(async (product) => {
          const { id } = product;
          const response = await fetch(`http://192.168.1.12:3000/carts/${id}`, {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  const postProductsToServer = async () => {
    try {
      ...
      const response = await fetch('http://192.168.1.12:3000/orders', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
